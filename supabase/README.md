...
3. Run the remaining migrations **in this order** — each is idempotent:
   `grant-privileges.sql` → `phase3-audit-log-policy.sql` → `phase4-tasks.sql` → `phase4-discussions.sql` → `roles-admin-viewer.sql` → **`roles-rls.sql`** → `phase5-sharepoint-sync.sql` → `phase5b-stock-safety-unit.sql` → `phase5c-production-from-inventory.sql` → `phase5d-archive.sql`.
   The `phase5*` files add columns the app already reads, so run them **before** deploying the app; a page that selects a column the database does not have shows its error state.
...
- **There is no hard delete.** `roles-rls.sql` sets `allow_delete = false` for every business table, and that is deliberate: `audit_log.entity_id` carries no foreign key, so deleting a record would strand its own Change Log history, and a voided tax invoice must stay on file. Records are retired with `phase5d-archive.sql`'s `archived_at` / `archived_by` instead, which the app writes as an ordinary UPDATE. Do not add DELETE policies.
