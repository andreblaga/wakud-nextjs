...
- [x] **Feedback module — built 2026-08-20.** `/feedback` with threaded replies over Realtime, triage behind admin/gm, conversion to a to-do item, and live notifications (untriaged items for admin/gm; unanswered replies for the submitter, derived from timestamps with no read-state table). Every signed-in role can submit and comment, `executive_viewer` included — the same deliberate exception Discussions makes, and asserted by a test. **Still open: the thread shows a short id for anyone who joins mid-session, because Realtime carries only an `author_id` and names are resolved server-side; it corrects itself on the next load.**
